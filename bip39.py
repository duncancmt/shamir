...
wordlist: list[str] = []
with open(
    os.path.join(os.path.dirname(__file__), "bip-0039.txt"), "r", encoding="utf-8"
) as f:
    for l in f:
...
def encode(entropy: bytes) -> tuple[str, ...]:
    checksum_bits = len(entropy) // 4
    checksummed = (int.from_bytes(entropy, "big") << checksum_bits) | checksum(entropy)
    result: list[str] = []
...
def decode(words: Iterable[str]) -> bytes:
    ...
    raw = functools.reduce(
        operator.or_, (x << (i * 11) for i, x in zip(range(len(raw) - 1, -1, -1), raw))
    )
